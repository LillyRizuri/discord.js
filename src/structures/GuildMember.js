...
class GuildMember extends Base {
	...
	_patch(data) {
		...
		if ('avatar' in data) {
			/**
			 * The ID of the members's avatar
			 * @type {?string}
			 */
		...
		this.timeout = data.communication_disabled_until === null ? false : new Date(data.communication_disabled_until);
	}
	...
	/**
	 * A link to the GuildMember's avatar.
	...
	avatarURL({ format, size, dynamic } = {}) {
		...
		return `https://cdn.discordapp.com/guilds/${this.guild.id}/users/${this.id}/avatars/${data?.avatar}.png`;
	}

	/**
	 * A link to the GuildMember's avatar if they have one.
	 * Otherwise a link to their default avatar will be returned.
	 * @param {ImageURLOptions} [options={}] Options for the Image URL
	 * @returns {string}
	 */
	displayAvatarURL(options) {
		return this.avatarURL(options) || this.user.displayAvatarURL({ size: 4096, dynamic: true });
	}
